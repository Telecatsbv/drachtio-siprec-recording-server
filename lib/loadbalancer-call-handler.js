...
const { getAvailableLoadBalancers, getRecorderPerDdi } = require('./utils');
...
function handleIncomingSiprecInvite(req, res, opts) {
  ...
  return payloadParser(opts)
    .then((opts) => {
      ...
      let recorderUri = getRecorderPerDdi(opts.callee.number);
      opts.logger.info(`RecorderUri: ${recorderUri}`);
      if (recorderUri === false) {
