...
module.exports = (req, res) => {
  ...
  // try {
  //   opts.record = config.get('rtpengine.record');
  //   opts.logger.info(`Using Config property record: ${opts.record}`);
  // } catch (err) {
  //   opts.logger.info(`Config property record not set: ${err}`);
  //   opts.record = 'no';
  // }
  // try {
  //   opts.mediaAddress = config.get('rtpengine.mediaAddress');
  //   opts.logger.info(`Using config property mediaAddress: ${opts.mediaAddress}`);
  // } catch (err) {
  //   opts.logger.info(`Config property mediaAddress not set: ${err}`);
  //   opts.mediaAddress = 'auto';
  // }
...
function allocateEndpoint(which, rtpEngine, opts) {
  ...
  const args = Object.assign({}, opts.callDetails, {
    ...
    'ICE': 'remove',
    'record call': rtpEngine.record
  });
  if (which === 'callee') Object.assign(args, {'to-tag': uuidv4()});
  if (rtpEngine.mediaAddress != 'auto') Object.assign(args, {'media address': rtpEngine.mediaAddress});
  // if (rtpEngine.mediaAddress != 'auto') console.log(`media address: ${rtpEngine.mediaAddress}`);

  debug(`callDetails: ${opts.callDetails}`);
  console.log(`rtpengine args for ${which}: ${JSON.stringify(args)}, sending to ${JSON.stringify(rtpEngine.remote)}`);
